...
class ColDesign:
    def __init__(self, EleTag, b, h, nbH, nbB, db, de, As, Pu_v, Mu_v, fiPn, fiMn, Mn_i, d, dist, ro, Mu_i,
                 ...
                 Vu_Vn, VnCol, sem, sigma_IF, wIF, Mpr_max):
        self.EleTag = EleTag
        self.b = b
        self.h = h
        self.nbH = nbH
...
class MyForm(QDialog):
    ...
    def CreateNLM(self):
        ...
        def YieldMomentColumn(h, b, dist_v, As_v, fc, Ec, P, fyL, Es):
            """
            Calcula el momento de fluencia (My) en una columna de concreto reforzado utilizando `fsolve`.
            
            Parámetros:
            h, b       -> Dimensiones de la columna (altura y ancho)
            dist_v     -> Distancias de las barras de refuerzo desde el borde superior
            As_v       -> Áreas de acero de cada nivel de barras
            fc, Ec     -> Resistencia y módulo de elasticidad del concreto
            P          -> Carga axial aplicada
            fyL, Es    -> Resistencia y módulo de elasticidad del acero
            
            Retorna:
            My         -> Momento de fluencia (kN·m)
            """

            # Parámetros auxiliares del concreto
            n_c = 0.8 + fc / 18.0  
            e_c = (fc / Ec) * (n_c / (n_c - 1.0))  # Deformación unitaria máxima del concreto

            # Curvatura de fluencia aproximada
            phiY = 2.1 * (fyL / Es) / h  

            # Función de equilibrio para encontrar c
            def equilibrium_condition(c):
                e_top = min(c * phiY, e_c)  # Límite de deformación del concreto

                # Centroide del diagrama de compresión parabólico
                cent_Fc = 1.0 - (8 * e_c - 3 * e_top) / (3 * e_c - e_top) / 4.0

                # Fuerza en el concreto
                Pconc = fc * b * c * ((e_top / e_c) - (e_top / e_c) ** 2 / 3.0)

                # Fuerza en el acero
                es = phiY * (c - dist_v)
                fs = np.clip(Es * es, -fyL, fyL)
                Pacer = np.sum(fs * As_v)

                return (Pconc + Pacer) - P  # Ecuación de equilibrio

            # Resolver c con `fsolve`
            c_guess = h / 2
            c_solution = fsolve(equilibrium_condition, c_guess)
            c = c_solution[0]

            # Verificar si la solución es válida
            if c < 0 or c > h:
                print("⚠️ Advertencia: No se encontró una solución física válida para c.")
                return None

            # Cálculo del momento
            e_top = min(c * phiY, e_c)
            cent_Fc = 1.0 - (8 * e_c - 3 * e_top) / (3 * e_c - e_top) / 4.0
            Pconc = fc * b * c * ((e_top / e_c) - (e_top / e_c) ** 2 / 3.0)
            es = phiY * (c - dist_v)
            fs = np.clip(Es * es, -fyL, fyL)
            ...
            Mconc = Pconc * (h / 2.0 - c * cent_Fc)

            # Momento del acero
            Macer = np.sum(fs * As_v * (h / 2.0 - dist_v))
        ...
        def split_and_sum(vec):
            """
            Splits a vector into two parts and sums each part.
            If the length is odd, the middle element is split equally between both parts.
            """
            n = len(vec)
            mid = n // 2  
            ...
            if n % 2 == 0:  
                ...
                vec1 = np.append(vec[:mid], vec[mid] / 2)
                vec2 = np.append(vec[mid] / 2, vec[mid+1:])

            return np.sum(vec1), np.sum(vec2)

        def ConcentratedHingeMaterial(h, b, dist_v, As_v, fc, Ec, P, fyL, Es, dst, nsb, dbL, sst, Lp, Mpr_p, Mpr_n, LEle):
            asl = 1  # Slip is possible
            Ag = b * h
            rho = np.sum(As_v) / Ag
            rho_t, rho_b = split_and_sum(As_v)  # Fixed variable name
            Ast = pi * dst ** 2 / 4
            Ash = nsb * Ast
            rho_sh = Ash / (b * max(sst, 1e-6))  # Avoid division by zero
            nu = P / (Ag * fc)
            Ig = b * h**3 / 12
            EA = Ec * Ag
            EIg = Ec * Ig

            EIstf_EIg = max(0.35, min(1.33 * (0.1 + nu)**0.80, 0.8))
            EIstf = EIstf_EIg * EIg
            # EIy_EIg = max(0.20, min(0.75 * (0.1 + nu)**0.80, 0.6))
            # EIy = EIy_EIg * EIg
            phiy = Mpr_p / EIstf
            My_n = Mpr_n
            My_p = Mpr_p    
            ...
            # phiy = 2.1 * fyL / Es / h
            # My_p = YieldMomentColumn(h, b, dist_v, As_v, fc, Ec, P, fyL, Es)
            # My_n = YieldMomentColumn(h, b, np.flip(h - dist_v), np.flip(As_v), fc, Ec, P, fyL, Es)
            
            Ki_p = My_p / phiy
            Ki_n = My_n / phiy
            Ery_p = Ki_p / EIg
            Ery_n = Ki_n / EIg
            Icr_p = Ery_p * Ig  # Fixed variable name
            Icr_n = Ery_n * Ig  # Fixed variable name

            Mc_My = (1.2**(0.89**nu)) * (0.91**(0.01 * fc/MPa))
            Mc_p = Mc_My * My_p
            Mc_n = Mc_My * My_n
            Mu_p = 0.1 * My_p
            Mu_n = 0.1 * My_n

            sn = sst / dbL * (fyL/MPa / 100) ** 0.5  # Buckling coefficient
            thetaCap = 0.12 * (1 + 0.55 * asl) * (0.16 ** nu) * ((0.02 + 40 * rho_sh) ** 0.43) * (0.54 ** (0.01 * fc/MPa)) * (0.66 ** (0.1 * sn)) * (2.27 ** (10 * rho))

            FardisT = ((max(0.01, rho_t * fyL / fc)) / (max(0.01, rho_b * fyL / fc))) ** 0.225
            thetaCap_p = thetaCap * FardisT
            thetaCap_n = thetaCap / FardisT

            thetaPC = min(0.76 * (0.031 ** nu) * (0.02 + 40 * rho_sh) ** 1.02, 0.10)
            phiCap_p = thetaCap_p / Lp + phiy
            phiCap_n = thetaCap_n / Lp + phiy
            phiUlt_p = phiCap_p + thetaPC / Lp
            phiUlt_n = phiCap_n + thetaPC / Lp

            d = dist_v[-1]
            lambda_d = 170.7 * (0.27 ** nu) * (0.1 ** (sst / max(d, 1e-6)))  # Avoid division by zero
            Lambda_S = lambda_d * phiy * Lp

            return My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S
        
        def create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiY, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, hingeMTag):
            """
            Creates a IMKPeakOriented material in OpenSeesPy with the given parameters.
            """
            # Compute Hardening Ratio
            Ksp1 = ((Mc_p - My_p) / (phiCap_p - phiY)) / My_p * phiY
            Ksn1 = ((Mc_n - My_n) / (phiCap_n - phiY)) / My_n * phiY

            # Material Model
            K0 = My_p / phiY  # Elastic stiffness
            as_Plus = Ksp1  # Strain hardening ratio for positive loading direction
            as_Neg = Ksn1  # Strain hardening ratio for negative loading direction
            My_Plus = My_p  # Effective yield strength for positive loading direction
            My_Neg = My_n  # Effective yield strength for negative loading direction (negative value)
            Lamda_S = Lambda_S  # Cyclic deterioration parameter for strength deterioration
            Lamda_C = Lambda_S  # Cyclic deterioration parameter for post-capping strength deterioration
            Lamda_A = 1.0 #0.0  # Cyclic deterioration parameter for acceleration reloading stiffness deterioration
            Lamda_K = 1.0 #0.0  # Cyclic deterioration parameter for unloading stiffness deterioration
            c_S = 1.0  # Rate of strength deterioration
            c_C = 1.0  # Rate of post-capping strength deterioration
            c_A = 1.0 #0.0  # Rate of accelerated reloading deterioration
            c_K = 1.0 #0.0  # Rate of unloading stiffness deterioration

            # Rotations
            phi_p_Plus = phiCap_p - phiY  # Pre-capping rotation for positive loading direction
            phi_p_Neg = phiCap_n - phiY  # Pre-capping rotation for negative loading direction
            phi_pc_Plus = phiUlt_p - phiCap_p  # Post-capping rotation for positive loading direction
            phi_pc_Neg = phiUlt_n - phiCap_n  # Post-capping rotation for negative loading direction

            # Residual strength ratio
            Res_Pos = Mu_p / My_p  # Residual strength ratio for positive loading direction
            Res_Neg = Mu_n / My_n  # Residual strength ratio for negative loading direction

            # Ultimate rotation capacity
            phi_u_Plus = 2 * phiUlt_p  # Ultimate rotation capacity for positive loading direction
            phi_u_Neg = 2 * phiUlt_n  # Ultimate rotation capacity for negative loading direction

            # Cyclic deterioration rate
            D_Plus = 1.0  # Cyclic deterioration rate in the positive loading direction
            D_Neg = 1.0  # Cyclic deterioration rate in the negative loading direction

            # Define material in OpenSeesPy
            # op.uniaxialMaterial("ModIMKPeakOriented", hingeMTag, K0, as_Plus, as_Neg, My_Plus, My_Neg,
            #                     Lamda_S, Lamda_C, Lamda_A, Lamda_K, c_S, c_C, c_A, c_K,
            #                     phi_p_Plus, phi_p_Neg, phi_pc_Plus, phi_pc_Neg,
            #                     Res_Pos, Res_Neg, phi_u_Plus, phi_u_Neg, D_Plus, D_Neg)

            # print("IMKPeakOriented", hingeMTag, K0, phi_p_Plus, phi_pc_Plus, phi_u_Plus, My_p, Mc_p, Mu_p, 
            #                     phi_p_Neg, phi_pc_Neg, phi_u_Neg, My_n, Mc_n, Mu_n, 
            #                     Lamda_S, Lamda_C, Lamda_A, Lamda_K, c_S, c_C, c_A, c_K, D_Plus, D_Neg)
            op.uniaxialMaterial("IMKPeakOriented", hingeMTag, K0, phi_p_Plus, phi_pc_Plus, phi_u_Plus, My_p, Mc_p/My_p, Mu_p/My_p, 
                        phi_p_Neg, phi_pc_Neg, phi_u_Neg, My_n, Mc_n/My_n, Mu_n/My_n, 
                        Lamda_S, Lamda_C, Lamda_A, Lamda_K, c_S, c_C, c_A, c_K, D_Plus, D_Neg)
            return
        ...
        for (Ele, DC, IFE, lwIF) in zip(EleCol, DataColDesign, infill_m, infill_lw):
            ...
            if self.ui.radioButtonFTCM.isChecked() == True:
                dp = DC.dist[0]
                dp1 = dp * FN
                y1 = DC.h / 2.0
                z1 = DC.b / 2.0 * FN
            ...
            if self.ui.radioButtonCHCM.isChecked() == True:
                My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DC.h, 
                DC.b * FN, DC.dist, DC.As * FN, fcC, Ele.EcEle, DC.NUD1, fy, Es, DC.de, DC.nsB, DC.db, DC.sst, phl, DC.Mpr_max, DC.Mpr_max, Ele.LEle)
                MatTag = Ele.EleTag * 20 + 14
                create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)
                # Create elastic section for the element
                Ag = DC.b * DC.h
                op.section('Elastic', Ele.EleTag * 10 + 1, Ele.EcEle, Ele.AEle, Ele.IzEle * FN)
                # Create uniaxial plastic hinge sections
                op.section('Uniaxial', Ele.EleTag * 10 + 2, MatTag, 'Mz')
                op.uniaxialMaterial('Elastic', Ele.EleTag * 20 + 13, Ele.EcEle * Ele.AEle * FN)
                # Create aggregated sections for the plastic hinges
                MassDens = Ele.AEle * GConc / g * FN
                op.section('Aggregator', Ele.EleTag * 10, Ele.EleTag * 20 + 13, 'P', '-section', Ele.EleTag * 10 + 2)
                op.beamIntegration('HingeRadau', Ele.EleTag * 2, Ele.EleTag * 10, phl, Ele.EleTag * 10, phl, Ele.EleTag * 10 + 1)
                op.element('forceBeamColumn', Ele.EleTag * 20, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2, '-mass', MassDens)
            ...
            if NIF != 0 and FNB > 0:
                ...
                if self.ui.radioButtonCHCM.isChecked() == True:
                    My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DC.h, 
                    DC.b * FNB, DC.dist, DC.As * FNB, fcC, Ele.EcEle, DC.NUD1 * FNB, fy, Es, DC.dst, DC.nsB, DC.db, DC.sst, phl, DC.Mpr_max * FNB, DC.Mpr_max * FNB, Ele.LEle)
                    MatTag = EleTag * 20 + 16
                    create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)
                    # Create elastic section for the element
                    Ag = DC.b * DC.h * FNB
                    op.section('Elastic', Ele.EleTag * 10 + 6, Ele.EcEle, Ele.AEle * FNB, Ele.IzEle * FNB)
                    # Create uniaxial plastic hinge sections
                    op.section('Uniaxial', Ele.EleTag * 10 + 7, MatTag, 'Mz')
                    op.uniaxialMaterial('Elastic', Ele.EleTag * 20 + 15, Ele.EcEle * Ele.AEle * FNB)
                    # Create aggregated sections for the plastic hinges
                    op.section('Aggregator', Ele.EleTag * 10 + 5, Ele.EleTag * 20 + 15, 'P', '-section', Ele.EleTag * 10 + 7)
                    op.beamIntegration('HingeRadau', Ele.EleTag * 2 + 1, Ele.EleTag * 10 + 5, phl, Ele.EleTag * 10 + 5, phl, Ele.EleTag * 10 + 6)
                    op.element('forceBeamColumn', Ele.EleTag * 20 + 10, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2 + 1, '-mass', MassDens)
        ...
        for (Ele, DB) in zip(EleBeam, DataBeamDesign):
            ...
            if self.ui.radioButtonCHCM.isChecked() == True:
                dst = 3 / 8 * inch
                dist_v1 = np.array([DB.h-DB.dt1, DB.db1])
                As_v1 = np.array([DB.Ast1, DB.Asb1])
                My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DB.h, 
                DB.b * FN, dist_v1, As_v1 * FN, fcC, Ele.EcEle, DB.NUD1, fy, Es, dst, DB.ns1, DB.db_t1, DB.ss1, phl1, DB.Mpr_n1, DB.Mpr_p1, Ele.LEle)
                MatTag = Ele.EleTag * 20 + 14
                create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)

                dist_v2 = np.array([DB.h-DB.dt2, DB.db2])
                As_v2 = np.array([DB.Ast2, DB.Asb2])
                My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DB.h, 
                DB.b * FN, dist_v2, As_v2 * FN, fcC, Ele.EcEle, DB.NUD1, fy, Es, dst, DB.ns2, DB.db_t2, DB.ss2, phl2, DB.Mpr_n2, DB.Mpr_p2, Ele.LEle)
                MatTag = Ele.EleTag * 20 + 15
                create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)
                
                # Create elastic section for the element
                Ag = DC.b * DC.h
                op.section('Elastic', Ele.EleTag * 10 + 2, Ele.EcEle, Ele.AEle, Ele.IzEle * FN)
                # Create uniaxial plastic hinge sections
                op.section('Uniaxial', Ele.EleTag * 10 + 3, Ele.EleTag * 20 + 14, 'Mz')
                op.section('Uniaxial', Ele.EleTag * 10 + 4, Ele.EleTag * 20 + 15, 'Mz')
                op.uniaxialMaterial('Elastic', Ele.EleTag * 20 + 16, Ele.EcEle * Ele.AEle * FN)
                # Create aggregated sections for the plastic hinges
                op.section('Aggregator', Ele.EleTag * 10, Ele.EleTag * 20 + 16, 'P', '-section', Ele.EleTag * 10 + 3)
                op.section('Aggregator', Ele.EleTag * 10 + 1, Ele.EleTag * 20 + 16, 'P', '-section', Ele.EleTag * 10 + 4)
                op.beamIntegration('HingeRadau', Ele.EleTag * 2, Ele.EleTag * 10, phl, Ele.EleTag * 10 + 1, phl, Ele.EleTag * 10 + 2)
                op.element('forceBeamColumn', Ele.EleTag * 20, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2, '-mass', MassDens)
            ...
            if NIF != 0 and FNB > 0:
                ...
                if self.ui.radioButtonFTCM.isChecked() == True:
                    Nod_ini = Ele.Nod_ini  # + nntot * 4
                    Nod_end = Ele.Nod_end  # + nntot * 5
                    op.beamIntegration('HingeRadau', Ele.EleTag * 2 + 1, Ele.EleTag * 10 + 5, phl1, Ele.EleTag * 10 + 6, phl2, Ele.EleTag * 10 + 5)
                    op.element('forceBeamColumn', Ele.EleTag * 20 + 10, Nod_ini + nnt1, Nod_end + nnt1, Ele.ElegTr, Ele.EleTag * 2 + 1, '-mass', MassDens)
                if self.ui.radioButtonCHCM.isChecked() == True:
                    My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DB.h, 
                    DB.b * FNB, dist_v1, As_v1 * FNB, fcC, Ele.EcEle, DB.NUD1 * FNB, fy, Es, dst, DB.ns1, DB.db_t1, DB.ss1, phl1, DB.Mpr_n1 * FNB, DB.Mpr_p1 * FNB, Ele.LEle)
                    MatTag = Ele.EleTag * 20 + 17
                    create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)

                    My_p, My_n, Mc_p, Mc_n, Mu_p, Mu_n, phiy, phiCap_p, phiCap_n, phiUlt_p, phiUlt_n, Lambda_S = ConcentratedHingeMaterial(DB.h, 
                    DB.b * FNB, dist_v2, As_v2 * FNB, fcC, Ele.EcEle, DB.NUD1 * FNB, fy, Es, dst, DB.ns2, DB.db_t2, DB.ss2, phl2, DB.Mpr_n2 * FNB, DB.Mpr_p2 * FNB, Ele.LEle)
                    MatTag = Ele.EleTag * 20 + 18
                    create_imk_material(Mc_p, My_p, phiCap_p, phiCap_n, phiy, Mc_n, My_n, Lambda_S, phiUlt_p, phiUlt_n, Mu_p, Mu_n, MatTag)
                    
                    # Create elastic section for the element
                    op.section('Elastic', Ele.EleTag * 10 + 7, Ele.EcEle, Ele.AEle, Ele.IzEle * FNB)
                    # Create uniaxial plastic hinge sections
                    op.section('Uniaxial', Ele.EleTag * 10 + 8, Ele.EleTag * 20 + 17, 'Mz')
                    op.section('Uniaxial', Ele.EleTag * 10 + 9, Ele.EleTag * 20 + 18, 'Mz')
                    op.uniaxialMaterial('Elastic', Ele.EleTag * 20 + 19, Ele.EcEle * Ele.AEle * FNB)
                    # Create aggregated sections for the plastic hinges
                    MassDens = Ele.AEle * GConc / g * FNB
                    op.section('Aggregator', Ele.EleTag * 10 + 5, Ele.EleTag * 20 + 19, 'P', '-section', Ele.EleTag * 10 + 8)
                    op.section('Aggregator', Ele.EleTag * 10 + 6, Ele.EleTag * 20 + 19, 'P', '-section', Ele.EleTag * 10 + 9)
                    op.beamIntegration('HingeRadau', Ele.EleTag * 2 + 1, Ele.EleTag * 10 + 5, phl, Ele.EleTag * 10 + 6, phl, Ele.EleTag * 10 + 7)
                    op.element('forceBeamColumn', Ele.EleTag * 20 + 10, Nod_ini + nnt1, Nod_end + nnt1, Ele.ElegTr, Ele.EleTag * 2 + 1, '-mass', MassDens)
